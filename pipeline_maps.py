...
def MakeRedshiftMap(inputdata,binmap):#regions-info-xspec-redshift.data
    if XSPEC:
        data = pd.read_csv(inputdata, delimiter=' ', index_col=0)
#        mapdata = data[0:168]
        mapdata = data
        region = [];nh = [];temp=[];temp_low=[];temp_high=[];temp_error_low=[];temp_error_high=[];temp_error_diff=[]
        abund=[];abund_low=[];abund_high=[];abund_error_low=[];abund_error_high=[];abund_error_diff=[];
        redshift=[];redshift_low=[];redshift_high=[];redshift_error_low=[];redshift_error_high=[];redshift_error_diff=[];
        norm=[];norm_low=[];norm_high=[];norm_error_low=[];norm_error_high=[];norm_error_diff=[]
        chi=[];dof=[];chi2=[]
        for index,row in mapdata.iterrows():
            region.append(index)
            ...
            abund_error_diff.append(round(row['abund_error_diff'],4))
            redshift.append(round(row['redshift'],4))
            redshift_low.append(round(row['redshift_low'],4))
            redshift_high.append(round(row['redshift_high'],4))
            redshift_error_low.append(round(row['redshift_error_low'],4))
            redshift_error_high.append(round(row['redshift_error_high'],4))
            redshift_error_diff.append(round(row['redshift_error_diff'],4))
            norm.append(round(row['norm'],4))
            norm_low.append(round(row['norm_low'],4))
            norm_high.append(round(row['norm_high'],4))
            norm_error_low.append(round(row['norm_error_low'],4))
...
#    LoadData(inputdata)
    ...
    for ii in region:
        imreg = imdata == ii
        boolmask.append((imreg.astype(np.int))*redshift[ii])
    
    redmap = sum(boolmask)
    #tempmap[tempmap >= 18] = np.nan
    _mkmap(redmap,"redshift_map.fits",hdr)
    #_printmap(tempmap,"temp_map.pdf")
    
    boolmask = []
    for ii in region:
        imreg = imdata == ii
        boolmask.append((imreg.astype(np.int))*redshift_error_diff[ii])
    
    rederrordepthmap = sum(boolmask)
    _mkmap((rederrordepthmap/2),"redshift_error_range_map.fits",hdr)
    #_printmap(temperrordepthmap,"temp_errordepthmap.pdf")
    ...
    #tempmap[tempmap >= 18] = np.nan
    _mkmap(redelowmap,"redshift_error_low_map.fits",hdr)
    ...
    #tempmap[tempmap >= 18] = np.nan
    _mkmap(redehighmap,"redshift_error_high_map.fits",hdr)

    _mkmap((rederrordepthmap/2)/redmap,"redshift_error_range_percent_map.fits",hdr)
